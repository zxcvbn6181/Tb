...
function decrypt(text) {
    let key_value = Crypto.enc.Utf8.parse('4OToScUFOaeVTrHE');
    let iv_value = Crypto.enc.Utf8.parse('9CLGao1vHKqm17Oz');
    let content = Crypto.AES.decrypt(text, key_value, {
...
async function searchContent(keyword) {
    try {
        ...
        const url = `https://yiso.fun/api/search?name=${encodedKeyword}&pageNo=1`;
        const html = await 访问网页(url, "GET","", yisoCookie);
        const items = JSON.parse(html);
        const list = items.data.list
        .map((item) => {
            if (item.from === 'quark') {
                const vod_name = item.name.replace(/<[^>]+>/g, ''); 
                const vod_pic = 'https://bizaladdin-image.baidu.com/0/pic/-1383952426_1737186201_-54798625.png';
                const vod_id = decrypt(item.url); 
                const vod_remarks = item.gmtCreate || '';
                return {
                    vod_id: vod_name+"||"+vod_id,
                    vod_name: vod_name,
...
async function detailContent(ids) {
    try {
      const [vod_name, vod_id] = ids.split("||");
      const defaultValues = {
        vod_year: '',
        vod_director: '未知',
        vod_actor: '未知',
      ...
      const movieDetails = {
        ...
        list: [{
          vod_id: ids,
          vod_name,
          vod_pic: defaultValues.vod_pic,
          vod_actor: defaultValues.vod_actor,
          vod_director: defaultValues.vod_director,
          vod_remarks: defaultValues.vod_remarks,
          vod_year: defaultValues.vod_year,
          vod_content: defaultValues.vod_content,
          vod_play_from,
...
const classData = [
    { "type_id": 1, "type_name": "搜索源无首页数据，只有历史和收藏记录" }
];
       const list = [{
           vod_id: '',
           vod_name: '搜索源无首页源',
